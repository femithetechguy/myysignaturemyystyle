...
const AUTO_GENERATED_FIELDS = ['id', 'created_at', 'updated_at', 'submission_id'];

export default function AdminContacts({ refreshKey = 0 }) {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [tableName, setTableName] = useState('contact_submissions');
  const [config, setConfig] = useState(staticConfig.admin.contacts || {});
    ...
    try {
      ...
      try {
        ...
        if (configRes.ok) {
          ...
          const tab = freshConfig.admin?.tabs?.find(t => t.id === 'contacts');
          setTableName(tab?.table || 'contact_submissions');
          setConfig(freshConfig.admin?.contacts || {});
  ...
  return (
    <div className="dashboardContainer">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', gap: '10px', flexWrap: 'wrap' }}>
        <h2 style={{ fontSize: '1.8rem', margin: 0, color: '#1B1B1B', flex: 1, minWidth: '150px' }}>{config.title || 'Manage Contacts'}</h2>
