...
const AUTO_GENERATED_FIELDS = ['id', 'created_at', 'updated_at', 'review_id'];

export default function AdminReviews({ refreshKey = 0 }) {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [tableName, setTableName] = useState('reviews');
  const [config, setConfig] = useState(staticConfig.admin.reviews || {});
    ...
    try {
      ...
      try {
        ...
        if (configRes.ok) {
          ...
          const tab = freshConfig.admin?.tabs?.find(t => t.id === 'reviews');
          setTableName(tab?.table || 'reviews');
          setConfig(freshConfig.admin?.reviews || {});
  ...
  return (
    <div className="dashboardContainer">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', gap: '10px', flexWrap: 'wrap' }}>
        <h2 style={{ fontSize: '1.8rem', margin: 0, color: '#1B1B1B', flex: 1, minWidth: '150px' }}>{config.title || 'Manage Reviews'}</h2>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 500 }}>
            ...
            <tbody>
              {sortedData.map((item, idx) => (
                <tr key={item.review_id || idx} style={{ borderBottom: '1px solid #eee' }}>
                  {columns.map(col => (
