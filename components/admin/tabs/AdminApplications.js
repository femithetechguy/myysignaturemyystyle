...
export default function AdminApplications() {
  ...
  const [tableName, setTableName] = useState('career_applications');
  const [config, setConfig] = useState(staticConfig.admin.applications || {});
    ...
    try {
      ...
      try {
        ...
        if (configRes.ok) {
          ...
          const tab = freshConfig.admin?.tabs?.find(t => t.id === 'applications');
          setTableName(tab?.table || 'career_applications');
          setConfig(freshConfig.admin?.applications || {});
